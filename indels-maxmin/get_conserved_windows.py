...
#load the nblrr csv
nblrr = open('nblrr-noscaffold-windows.csv', 'r')
nblrrcsv = csv.reader(nblrr, delimiter='\t', quotechar=None, doublequote=False)
winlist = [row for row in nblrrcsv]
...
def make_list(a, start, stop, n=20):
	"""a is a list of pairs of integers, (x, len) with x the start of a
        ...
        n is the minimum length of a conserved region to be considered 

	output is a list of tuples which describe intervals in [start, stop]
	which don't contain the elements of a, given in form (start, stop)
...
for row in winlist[1:]:
	...
	for name in inputlist:
		...
		for en in [x for x in vcfcsv if len(x)>2 and x[0][0] != "#" and int(x[0][-2:]) == chrom and start-offset <= int(x[1]) <= stop+offset]:
                        a = en[4].split(",")
